...
#define FNAME_OUTIIBS "../include/def68k-iibs.h"
#define FNAME_OUTFUNCS "../include/def68k-funcs.h"
#define FNAME_OUTPROTO "../include/def68k-proto.h"
             ...
             ((t_datatype)ea_f == (t_datatype)ea_Dreg) || (num[bitf] && ((t_datatype)ea_f <= (t_datatype)ea_Imm)); ea_f++)
        {
          ...
          for (cc = 0; (cc == 0) || (num[bitc] && (cc < 16)); cc++)
          {
            ...
            case i_EOR:
              ...
              {                              //
                if ((bits & 0xffb0) == 0xa030)
                  clocks = 20; // EORI to CCR/SR
                else
                { //
                  if (dtype == dt_Dreg)
                    clocks = (size != sz_long ? 8 : 12); // EORI D0-D7
                  else
                    clocks = (size != sz_long ? 16 : 20) + clocks_eacalc(dtype, size); // EORI Memory
                }                                                                      //
              }                                                                        // EORI RA2005.05.06
              else
              {
            ...
            case i_AND:
              ...
              {                                                       //
                if ((bits & 0xffbf) == 0x003c || (bits & 0xffbf) == 0x023c)
                  clocks = 20; // to CCR/SR
                {              //
                  if (dtype == dt_Dreg)
                    clocks = (size != sz_long ? 8 : 12); // D0-D7
                  else
                    clocks = (size != sz_long ? 16 : 20) + clocks_eacalc(dtype, size); // Memory
                }                                                                      //
            ...
            case i_CMPA:
              ...
              {                                //
                if (dtype == dt_Dreg)
                  ...
                  clocks = (size != sz_long ? 8 : 12) + clocks_eacalc(dtype, size); // Memory
              }                                                                     //
              else
              {
                if (dtype == dt_Areg)
            ...
            case i_BSET:
              if (stype == dt_Dreg)
              {                                                                   /* dynamic */
                clocks = (dtype == dt_Dreg) ? 8 : 8 + clocks_eacalc(dtype, size); // RA2005.05.07 see below
              }
              else
              {                                                                     /* static */
                clocks = (dtype == dt_Dreg) ? 14 : 12 + clocks_eacalc(dtype, size); // RA2005.05.07 see below
              }
              break; // 20191127
            case i_BCLR:
              if (stype == dt_Dreg)
              {                                                                    /* dynamic */
                clocks = (dtype == dt_Dreg) ? 10 : 8 + clocks_eacalc(dtype, size); // RA2005.05.07 see below
              }
              else
              {                                                                     /* static */
                clocks = (dtype == dt_Dreg) ? 14 : 12 + clocks_eacalc(dtype, size); // RA2005.05.07 see below
            ...
            case i_BTST:
              if (stype == dt_Dreg)
              {                                                                   /* dynamic */
                clocks = (dtype == dt_Dreg) ? 6 : 4 + clocks_eacalc(dtype, size); // was stype, changed to dtype RA2005.05.07 because 4+ is on the else part of ?
              }
              else
              {                                                                    /* static */
                clocks = (dtype == dt_Dreg) ? 10 : 8 + clocks_eacalc(dtype, size); // ditto - wonder if it applies to the above too, not just BTST???
              }
              break;
            case i_JMP:
              switch (stype)
            ...
            case i_MOVEFSR:
              ...
              break;
            case i_MOVETSR:
              clocks = (stype == dt_Dreg) ? 12 : 12 + clocks_eacalc(stype, size);
              break;
            case i_MOVETUSP:
            case i_MOVEFUSP:
              clocks = 4;
...
int clocks_eacalc(t_datatype type, t_size size)
{
  ...
  {
  case dt_Dreg:
    fprintf(myoutiibs, " /*Dreg0*/     ");
    return 0;
  case dt_Areg:
    fprintf(myoutiibs, " /*Areg0*/     ");
    return 0;
  case dt_Aind:
    fprintf(myoutiibs, " /*Aind4:8*/   ");
    return (size != sz_long) ? 4 : 8;
  case dt_Ainc:
    fprintf(myoutiibs, " /*Ainc4:8*/   ");
    return (size != sz_long) ? 4 : 8; /* should this be 4/8? */ // RA was 6:10 - IMHO 4/8
  case dt_Adec:
    fprintf(myoutiibs, " /*Adec6:10*/  ");
    return (size != sz_long) ? 6 : 10;
  case dt_Adis:
    fprintf(myoutiibs, " /*Adis8:12*/  ");
    return (size != sz_long) ? 8 : 12;
  case dt_Aidx:
    fprintf(myoutiibs, " /*Aidx10:14*/ ");
    return (size != sz_long) ? 10 : 14;
  case dt_AbsW:
    fprintf(myoutiibs, " /*AbsW8:12*/  ");
    return (size != sz_long) ? 8 : 12;
  case dt_AbsL:
    fprintf(myoutiibs, " /*AbsL12:16*/ ");
    return (size != sz_long) ? 12 : 16;
  case dt_Pdis:
    fprintf(myoutiibs, " /*Pdis8:12*/  ");
    return (size != sz_long) ? 8 : 12;
  case dt_Pidx:
    fprintf(myoutiibs, " /*Pidx10:14*/ ");
    return (size != sz_long) ? 10 : 14;
  case dt_ImmB:
    fprintf(myoutiibs, " /*ImmB4:8*/   ");
    return (size != sz_long) ? 4 : 8;
  case dt_ImmW:
    fprintf(myoutiibs, " /*ImmW4:8*/   ");
    return (size != sz_long) ? 4 : 8;
  case dt_ImmL:
    fprintf(myoutiibs, " /*ImmL4:8*/   ");
    return (size != sz_long) ? 4 : 8;
  case dt_ImmS:
    fprintf(myoutiibs, " /*ImmS:0*/    ");
    return 0;
  case dt_Imm3:
    fprintf(myoutiibs, " /*Imm3:0*/    ");
    return 0;
  case dt_Imm4:
    fprintf(myoutiibs, " /*Imm4:0*/    ");
    return 0;
  case dt_Imm8:
    fprintf(myoutiibs, " /*Imm8:0*/    ");
    return 0;
  case dt_Imm8s:
    fprintf(myoutiibs, " /*Imm8:0*/    ");
    return 0;
  case dt_ImmV:
    fprintf(myoutiibs, " /*ImmV:0*/    ");
    return 0;
  case dt_Ill:
    fprintf(myoutiibs, " /*Ill :0*/    ");
    return 0;
  }
  printf("Invalid type for clocks\n");
  exit(1);
