...
void GTerm::ProcessInput(int len, unsigned char *data)
{
	...
	while (data_len)
	{
		...
		while (current_state[i].byte != -1 &&
			   current_state[i].byte != *input_data)
			i++;
	...
	if (!(mode_flags & DEFERUPDATE) ||
		(pending_scroll > scroll_bot - scroll_top))
		update_changes();
}
...
#ifdef GTERM_PC
void GTerm::SetMachineName(char *machinename)
{
	if (pc_machinename)
		delete pc_machinename;
...
int GTerm::IsSelected(int x, int y)
{
...
void GTerm::Select(int x, int y, int select)
{
	if (color && x >= 0 && x < Width() && y >= 0 && y < Height())
	{
		if (select)
